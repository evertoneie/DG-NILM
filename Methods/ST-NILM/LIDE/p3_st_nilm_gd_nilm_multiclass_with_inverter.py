...
configs = {
    ...
    "DATASET_PATH": "",
    "TRAIN_SIZE": 0.9,
    "FOLDER_PATH": "./ST-NILM/LIDE/trained_models/",
    "FOLDER_DATA_PATH": "ST-NILM/LIDE/trained_models/",
...
if not load_existing_segments:
    ...
    if normalized:
        file_name2 = "normalized_Detect_Multiclass_Inverter"
    else:
        file_name2 = "Detect_Multiclass_Inverter"
...
def buildBaseScattering_2order(input_shape, J=J, Q=Q, start1=start1, end1=end1, start2=start2):
    ...
    x_class = tf.concat(
        [leftav1, g1av1, g2av1, g3av1, g4av1, g5av1, rightav1, leftav2, g1av2, g2av2, g3av2, g4av2, g5av2, rightav2],
        axis=1)
...
def buildBaseScattering(input_shape, J, Q):
    ...
    x = Scattering1D(J, Q, max_order=1)(input) 
    ...
    # We removed the original event type classification
    model = Model(inputs = input, outputs=[x_class])
...
for train_index, validation_index in mskf.split(X_all, strat_classes):
    ...
    # updating custom loss list
    keras.utils.get_custom_objects().update({'focal_loss': ModelHandler.multi_label_focal_loss()})
    keras.utils.get_custom_objects().update({'bce_weighted_loss': ModelHandler.get_bce_weighted_loss(yclass_weights)})
    keras.utils.get_custom_objects().update({'loss': ModelHandler.weighted_categorical_crossentropy(None)})
    keras.utils.get_custom_objects().update({'dice_loss': ModelHandler.dice_loss_multiclass()})
    keras.utils.get_custom_objects().update({'generalized_dice_loss': ModelHandler.generalized_dice_loss()})
    keras.utils.get_custom_objects().update({'jaccard_loss': ModelHandler.jaccard_loss_multiclass()})

    if not os.path.exists(source_models_folder + 'ST_NILM' +  '/' + '_J' + str(J) + '_Q' + str(Q) + complemento2 + '/' + str(fold)):
                  
                # if the demo_folder directory is not present 
                # then create it.
        os.makedirs(source_models_folder + 'ST_NILM' +  '/' + '_J' + str(J) + '_Q' + str(Q) + complemento2 + '/' + str(fold))


    if not os.path.exists(source_models_folder + 'ST_NILM' +  '/' + '_J' + str(J) + '_Q' + str(Q) + complemento2 + '/logs'):

        os.makedirs(source_models_folder + 'ST_NILM' +  '/' + '_J' + str(J) + '_Q' + str(Q) + complemento2 + '/logs')

    np.save(source_models_folder + 'ST_NILM' +  '/' + '_J' + str(J) + '_Q' + str(Q) + complemento2 + '/' + str(fold) + "/train_index.npy", train_index)
    np.save(source_models_folder + 'ST_NILM' +  '/' + '_J' + str(J) + '_Q' + str(Q) + complemento2 + '/' + str(fold) + "/validation_index.npy", validation_index)
    

    tensorboard_callback = TensorBoard(log_dir=source_models_folder + 'ST_NILM' +  '/' + '_J' + str(J) + '_Q' + str(Q) + complemento2 + '/logs')
    ...
    while fileEpoch < configs["TOTAL_MAX_EPOCHS"]:
        ...
        if not os.path.isfile(folderPath + 'Detect_Multiclass_Inverter_all_512.h5'):
            ...
            for subtask in ['classification']:
                ...
                early_stopping_callback = EarlyStopping(monitor="val_binary_accuracy", patience=50, verbose=True, restore_best_weights=True)
            ...
            if os.path.exists(source_models_folder + 'ST_NILM' +  '/' + '_J' + str(J) + '_Q' + str(Q) + complemento2 +'/' + str(fold) + '/' + complemento + '.h5'):
                os.remove(source_models_folder + 'ST_NILM' +  '/' + '_J' + str(J) + '_Q' + str(Q) + complemento2 + '/' + str(fold) + '/' + complemento + '.h5')
                print('The model exists! We replaced it!')


            # Then save the new trained model
            model.save(source_models_folder + 'ST_NILM' +  '/' + '_J' + str(J) + '_Q' + str(Q) + complemento2 + '/' + str(fold) + '/' + complemento + '.h5')
