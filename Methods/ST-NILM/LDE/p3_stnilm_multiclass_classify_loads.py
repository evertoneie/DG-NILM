...
configs = {
    ...
    "TRAIN_SIZE": 0.9,
    "FOLDER_PATH": "/media/everton/Dados_SATA/Artigo_Dataset/Colab/ST-NILM/SC6_Classify_Loads/trained_models/",
    "FOLDER_DATA_PATH": "/media/everton/Dados_SATA/Artigo_Dataset/Colab/ST-NILM/SC6_Classify_Loads/trained_models/",
...
def windows(data, window_size):  # aqui define-se o tamanho do janelamento...
    start = 0
...
def extract_segments2(data, window_size=30):
    segments = np.empty((0, (window_size), 1))  # tantas colunas quanto tamanho da janela...
    labels = np.empty((0, 4))
    for (start, end) in windows(data,
                                window_size):  # essa função windows atua como um gerador, já que tem um yield lá dentro (esse for gera o próximo elemento da lista)
        if (len(data["data"][0, start:end, 0]) == (window_size)):
            signal = data["data"][:, start:end,
                     :]  # a dimensão 1 é referente ao tempo.. a zero representam os exemplos, e a 2 são as diferentes séries temporais
            segments = np.vstack([segments, signal])
            # labels = np.append(labels,stats.mode(data["labels"][start:end])) # O que fiz aqui foi só trocar as estruturas para compatibilizar com os meus dados
            # labels = np.append(labels,data["labels"][start:end]) # O que fiz aqui foi só trocar as estruturas para compatibilizar com os meus dados
            labels = np.vstack([labels, data["labels"]])  # O que vamos ter que fazer é empilhar as labelss

    return segments, labels  # O que essa função aparenta fazer é criar uma estrutura de dados com stride de window_size/2, semelhante ao que fizemos.

    # tendo isso em vista, achei melhor


def extract_segments(data, window_size=30):
    segments = np.empty(
        (0, (window_size + 1)))  # tantas colunas quanto tamanho da janela... tantas linhas quanto exemplos
    labels = np.empty((0))
    ...
    return segments, labels  # no fim essa função faz algo parecido com o que fiz: recorta as janelas de tempo válidas... eu considerei um código separado pra isso
...
for subsets_selector in sub_selector:
    ...
    if subsets_selector == 0:
        complemento = "Detect_Multiclass_Classify_Loads_aggregated_512"
        complemento2 = "_aggregated"
    elif subsets_selector == 1:
        complemento = "Detect_Multiclass_Classify_Loads_individual_512"
        complemento2 = "_individual"
    elif subsets_selector == 2:
        complemento = "Detect_Multiclass_Classify_Loads_all_512"
        complemento2 = "_all"
    elif subsets_selector == 3:
        complemento = "Detect_Multiclass_Classify_Loads_all_with_inverter_512"
        complemento2 = "_all_with_inverter"
    elif subsets_selector == 4:
        complemento = "Detect_Multiclass_Classify_Loads_all_without_inverter_512"
        complemento2 = "_all_without_inverter"
    elif subsets_selector == 5:
        complemento = "Detect_Multiclass_Classify_Loads_individual_with_inverter_512"
        complemento2 = "_individual_with_inverter"
    elif subsets_selector == 6:
        complemento = "Detect_Multiclass_Classify_Loads_individual_without_inverter_512"
        complemento2 = "_individual_without_inverter"
    elif subsets_selector == 7:
        complemento = "Detect_Multiclass_Classify_Loads_aggregated_with_inverter_512"
        complemento2 = "_aggregated_with_inverter"
    elif subsets_selector == 8:
        complemento = "Detect_Multiclass_Classify_Loads_aggregated_without_inverter_512"
    ...
    if not load_existing_segments:
        ...
        for subset_type in options:
            ...
            # data = {}
            # sio.loadmat(root_path + '/data.mat', mdict=data)
        ...
        if reduced_dataset_flag:
            ...
            train_x, val_x, train_y, val_y = train_test_split(Reshaped_segments, Labels,
                                                              stratify=Labels,
                                                              test_size=0.5)  # Estou reduzindo pela terça parte o conjunto de treino e teste
        ...
        if normalized:
            file_name2 = "normalized_Detect_Multiclass_Classify_Loads"
        else:
            file_name2 = "Detect_Multiclass_Classify_Loads"
        ...
        # segments_path = root_path + "/segments/" + file_name2
        ...
        # remove_if_exists(segments_path + '/' + "test_x.mat")
        # remove_if_exists(segments_path + '/' + "train_y.mat")
        # remove_if_exists(segments_path + '/' + "test_y.mat")
        ...
        # sio.savemat(root_path + '/' + "train_y.mat", savedict_train_y)
        # sio.savemat(root_path + '/' + "test_x.mat", savedict_test_x)
        # sio.savemat(root_path + '/' + "test_y.mat", savedict_test_y)
    ...
    if load_existing_segments:
        ...
        # segments_path = LSTM_TR_path + "/segments/" + file_name
    ...
    # train_x = segments_data['train_x'].reshape([segments_data['train_x'].shape[0],-1])
    train_y = segments_data["train_y"]
    test_x = segments_data["test_x"]
    # test_x = segments_data['test_x'].reshape([segments_data['test_x'].shape[0],-1])
    ...
    X_all = train_x
    yclass = train_y  # shape (n_examples,n_classes) - one hot encoding
    ...
    def buildBaseScattering(input_shape, J, Q):
        ...
        x = Lambda(lambda x: tf.math.log(tf.abs(x) + log_eps))(x)  # evitar explosão
    ...
    for train_index, validation_index in mskf.split(X_all, strat_classes):
        ...
        print("Numero de exageros na validação:", np.sum(validation_index > train_x.shape[0]))
        print("Numero de exageros no treino:", np.sum(train_index > train_x.shape[0]))
        ...
        print("Shape of x_train before ST: ", x_train.shape)
        print("Shape of x_validation before ST: ", x_validation.shape)
        ...
        print("Shape of x_train before ST: ", x_train_class.shape)
        print("Shape of x_validation before ST: ", x_validation_class.shape)

        # Normalizing
        nan_x_train = np.count_nonzero(np.isinf(x_train_class))
        nan_x_validation = np.count_nonzero(np.isinf(x_validation_class))

        print("We have ", nan_x_train, " infinity positions in x_train_class")
        print("We have ", nan_x_validation, " infinity positions in x_validation_class")
