...
configs = {
    ...
    "TRAIN_SIZE": 0.9,
    "FOLDER_PATH": "/media/everton/Dados_SATA/Artigo_Dataset/Colab/DeepDFML/SC6_Classify_Loads/trained_models/",
    "FOLDER_DATA_PATH": "/media/everton/Dados_SATA/Artigo_Dataset/Colab/DeepDFML/SC6_Classify_Loads/trained_models/",
...
for subsets_selector in sub_selector:
    ...
    def buildBaseScattering_2order(input_shape, start1, end1, start2, J=J, Q=Q):
        '''
    ...
    if train_flag:
        for train_index, validation_index in mskf.split(X_all, strat_classes):
            ...
            scaler = StandardScaler()
            scaler.fit(np.squeeze(X_all[validation_index], axis=2))
            ...
            if not os.path.exists(
                    source_models_folder + 'DeepDFML' + '/' + complemento2 + '/' + str(fold)):
                # if the demo_folder directory is not present
                # then create it.
                os.makedirs(
                    source_models_folder + 'DeepDFML' + '/' + complemento2 + '/' + str(fold))
            ...
            if not os.path.exists(
                    source_models_folder + 'DeepDFML' + '/' + complemento2  + '/logs'):
                os.makedirs(source_models_folder + 'DeepDFML' + '/' + complemento2  + '/logs')

            np.save(source_models_folder + 'DeepDFML' + '/' + complemento2  + '/' + str(
                fold) + "/train_index.npy", train_index)
            np.save(source_models_folder + 'DeepDFML' + '/' + complemento2  + '/' + str(
                fold) + "/validation_index.npy", validation_index)

            tensorboard_callback = TensorBoard(
                log_dir=source_models_folder + 'DeepDFML' + '/' + complemento2  + '/logs')

            if configs["INITIAL_EPOCH"] > 0:
                model = ModelHandler.loadModel(folderPath + 'model_{0}.h5'.format(configs["INITIAL_EPOCH"]))
            else:
                model = modelHandler.buildModel2(input_shape=X_all.shape[1])
            ...
            while fileEpoch < configs["TOTAL_MAX_EPOCHS"]:
                ...
                if not os.path.isfile(folderPath + 'Detect_Multiclass_Classify_Loads_all_512.h5'):
         ...
         \
                    ...
                    if os.path.exists(
                            source_models_folder + 'DeepDFML' + '/' + complemento2 + '/' + str(
                                    fold) + '/' + complemento + '.h5'):
                        os.remove(
                            source_models_folder + 'DeepDFML' + '/' + complemento2 + '/' + str(
                                fold) + '/' + complemento + '.h5')
            ...
            del model, y_validation, y_train, x_train_class, x_validation_class
